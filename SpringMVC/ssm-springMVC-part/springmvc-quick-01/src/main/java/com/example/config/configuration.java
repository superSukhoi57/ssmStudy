...
/*
FIXME：整合mybatis时配置类要写的东西
持久层配置信息、连接池、sqlSessionFactory、Mapper代理对象！
 */
...
//@PropertySource("classpath:db.properties")//配置mybatis时引入配置文件。
//FIXME：配置类，配置springmvc
@EnableWebMvc//FIXME：加这个注解相当于给handelerMapping、handlerAdaprter配置了json转换器！
@Configuration
@ComponentScan({"com.example.controller","com.example.allErrorException","com.example.exceptions","com.example.service"})
...
public class configuration  implements WebMvcConfigurer {
    ...
    public TransactionManager transactionManager(DataSource dataSource){
        ...
        dataSourceTransactionManager.setDataSource(dataSource);//给事务配置一个连接池！
